...
FROM orders O
JOIN product P ON O.PRICING_ID = P.PRICING_ID
WHERE O.CUS_ID = 2;
...
-- 6) Find the least expensive product from each 
-- category and print the table with category id, name, 
...
BEGIN
    ...
    FROM supplier S
    JOIN supplier_pricing SP ON S.SUPP_ID = SP.SUPP_ID
    JOIN rating R ON SP.PRICING_ID = R.PRICING_ID;
END //
DELIMITER ;
